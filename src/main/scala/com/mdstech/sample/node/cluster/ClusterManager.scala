package com.mdstech.sample.node.cluster

import akka.actor.{Actor, ActorLogging, ActorRef, Props}
import akka.cluster.{Cluster, MemberStatus}
import com.mdstech.sample.node.cluster.ClusterManager.GetMembers
...
class ClusterManager(nodeId: String) extends Actor with ActorLogging {
  ...
  override def receive: Receive = {
    case GetMembers => {
      sender() ! cluster.state.members.filter(_.status == MemberStatus.Up).map(_.address.toString).toList
    }
  }
}
